...
- `execute`: consente di eseguire comandi se sono soddisfatte certe condizioni e di spostare il contesto di esecuzione da un entità a un altra o da una coordinata a un altra. `execute as @a at @s run setblock ~ ~-1 ~ stone` esegue i seguenti passi
	- seleziona tutti i giocatori e imposta il giocatore come entità che sta eseguendo il comando (`as @a`)
...
Il dominio delle operazioni tra scoreboard è $bb(z) inter [-2^{31}, 2^{31}-1]$. Divisioni che avrebbero risultati decimali vengono approssimate all'intero più vicino. Ad esempio se si vuole calcolare il 5% di 40, bisogna stare attenti all'ordine delle operazioni:
...
```
$=0$ dato che $40/100=0$, mentre
```c
scoreboard players operation set $temp math 40
scoreboard players operation $temp math *= #5 math
scoreboard players operation $temp math /= #100 math
```
$=2$, dato che $40 times 5>100$, quindi dividendolo per $100$ non verrà approssimato a 0.
...
```
Tuttavia questo metodo può essere problematico se scalando, il valore è maggiore di $2^{31}-1$. Se $"mana"=4100000$ e $"max mana"=5100000$,
$"mana_perc"=-3.82$.
