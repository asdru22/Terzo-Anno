...
Un _activity partition_ è un gruppo di attività per azioni che hanno una caratteristica comune. Le partizioni forniscono una visualizzazione ristretta dei comportamenti chiamati in attività e spesso corrispondono a unità organizz azionali. Ogni corsia è chiamata _swim lane_.
...
Le _event actions_ sono usate per modellare interazioni che avvengono al difuori dell'attività corrente. Produrre eventi è asincrono, consumare eventi è sincrono (bloccante). Le azioni ad evento sono:
- invio segnale
- accetta segnare
...
==== Storia degli stati
La _state history_ è usata per tenere traccia dell'avanzamento dello stato di una regione. La regione può essere resettata da una transizione locale che si connette a uno pseudostato di tipo _histotry_.
 = Transizioni
...
=== Run-to-completion
Una state machine viene creata, e poi inizializzata, eseguendo la transizione composta iniziale. Successivamente entra in un _wait point_. Quando gli eventi sono inviati, i trigger vengono valutati, e se almeno una transizione può essere eseguita, una nuova transizione composta viene eseguita, e si raggiunge un nuovo punto di attesa. Questo ciclo si ripete finché la state machine completa il suo comportamento o finché è asincronicamente terminato da un agente esterno. Questo modello è chiamato _run to completion_ (RTC).
...
_Soluzione_: si assegnano le responsabilità affinché l'accopiamento tra le parti rimanga basso.
=== High Cohesion
_Problema_: come si mantengono gli oggetti concentrati, comprensibili e gestibili e supportabili per il _low coupling_?
...
=== Pure Fabrication
_Problema_: cosa si fa quado non si vuole violare high cohesion e low coupling, ma soluzioni offerte dall'Expert non sono corrette?
