...
/ Riassumendo: Data una chiave minimale $K$ e dipendenze $X->Y$ di uno schema $R(X)$,
$
&forall X->Y, K subset.eq X and Y eq.not K &=> "2FN"  \
&forall X, K subset X & => "FNBC" \
&lr((R(X) in "FNBC"), size:#150%) or lr((forall X->Y, Y subset K or K subset X),size:#150%) & => "3FN"  \
$
